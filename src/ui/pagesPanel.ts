...
export function buildPagesPanel(host: HTMLElement, renderer: Renderer, requestRender: () => void): void {
  const render = (): void => {
    host.innerHTML = '';
    ...
    opt0.value = ''; opt0.textContent = '— blank —';
    ...
    const lt = document.createElement('span'); lt.textContent = 'New page from';
    tplLabel.appendChild(lt); tplLabel.appendChild(tplSelect);
    ...
    store.doc.pages.forEach((p, i) => {
      ...
      name.addEventListener('change', () => { store.transact(() => { p.name = name.value; }); });
      meta.appendChild(name);
      const tplName = store.doc.templates.find(t => t.id === p.templateId)?.name;
      ...
      actions.appendChild(iconBtn('▲', 'Move up', () => { store.movePage(p.id, -1); requestRender(); }));
      actions.appendChild(iconBtn('▼', 'Move down', () => { store.movePage(p.id, 1); requestRender(); }));
      actions.appendChild(iconBtn('×', 'Delete', () => {
        if (store.doc.pages.length > 1 && confirm(`Delete ${p.name}?`)) {
          store.deletePage(p.id); renderer.invalidate(); requestRender();
        }
      }));
    ...
    saveBtn.addEventListener('click', () => {
      const name = prompt('Template name:', `Template ${store.doc.templates.length + 1}`);
      if (!name) return;
    ...
    if (!store.doc.templates.length) {
      ...
      empty.textContent = 'No templates yet. Save the current page as a template to reuse its layout.';
    ...
    for (const t of store.doc.templates) {
      ...
      row.appendChild(n);
      row.appendChild(iconBtn('Apply', 'Apply to current page', () => {
        store.applyTemplate(t.id); renderer.invalidate(); requestRender();
      }, 'small'));
      row.appendChild(iconBtn('×', 'Delete template', () => {
        if (confirm(`Delete template ${t.name}?`)) { store.deleteTemplate(t.id); }
      }, 'small'));
...
function iconBtn(label: string, title: string, onClick: () => void, extra = ''): HTMLButtonElement {
