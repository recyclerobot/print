...
export function createRulers(host: HTMLElement, renderer: Renderer): RulerHandles {
  ...
  function draw(): void {
    ...
    top.width = wPx; top.height = Math.floor(RW * dpr);
    top.style.width = `${r.width - RW}px`; top.style.height = `${RW}px`;
    top.style.left = `${RW}px`; top.style.top = '0';
    left.width = Math.floor(RW * dpr); left.height = hPx;
    left.style.width = `${RW}px`; left.style.height = `${r.height - RW}px`;
    left.style.left = '0'; left.style.top = `${RW}px`;
...
function drawAxis(canvas: HTMLCanvasElement, orient: 'h' | 'v', dpr: number, renderer: Renderer): void {
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  if (orient === 'h') { ctx.moveTo(0, H - 0.5); ctx.lineTo(W, H - 0.5); }
  ...
  let unitMm = 1;
  switch (unit) {
    case 'mm': unitMm = 1; break;
    case 'cm': unitMm = 10; break;
    case 'in': unitMm = 25.4; break;
    case 'pt': unitMm = 25.4 / 72; break;
  }
  ...
  for (const c of candidates) {
    if (c * pxPerUnit >= targetPx) { step = c; break; }
    step = c;
  }
  const minor = step / 5;
  ...
  // Highlight page area
  const pageStart = 0;
  const pageEnd = orient === 'h' ? renderer.doc.size.width : renderer.doc.size.height;
