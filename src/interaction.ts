...
export function attachInteraction(canvas: HTMLCanvasElement, renderer: Renderer, requestRender: () => void): void {
  const drag: DragState = { mode: null, startX: 0, startY: 0, startMm: { x: 0, y: 0 }, initial: new Map() };
  let snapLines: SnapLine[] = [];

  function hitTest(mx: number, my: number): { el?: AnyElement; handle?: 'nw' | 'ne' | 'sw' | 'se'; handleId?: string } {
  ...
  canvas.addEventListener('mousedown', (e) => {
    ...
    if (hit.handle && hit.handleId) {
      ...
      for (const id of store.selectedIds) {
        ...
        if (el) drag.initial.set(id, { x: el.x, y: el.y, w: el.width, h: el.height });
      }
      store.pushHistory();
    } else {
    ...
    if (drag.mode === 'move') {
      ...
      if (store.prefs.snapEnabled && drag.initial.size) {
        ...
        for (const [id, init] of drag.initial) {
          const x = init.x + dx, y = init.y + dy;
          bx0 = Math.min(bx0, x); by0 = Math.min(by0, y);
          bx1 = Math.max(bx1, x + init.w); by1 = Math.max(by1, y + init.h);
          void id;
        }
        const snap = computeSnap(bx0, by0, bx1, by1, store.doc, page.elements, movingIds, store.prefs.snapTolerance / renderer.view.zoom);
      ...
      for (const [id, init] of drag.initial) {
        const el = page.elements.find(p => p.id === id);
        if (el) { el.x = init.x + dx; el.y = init.y + dy; }
      }
      store.emit();
      requestRender();
      ...
      const mx = mm.x, my = mm.y;
      if (drag.resizeHandle.includes('e')) nw = Math.max(2, mx - init.x);
      if (drag.resizeHandle.includes('s')) nh = Math.max(2, my - init.y);
      if (drag.resizeHandle.includes('w')) { nx = mx; nw = Math.max(2, init.x + init.w - mx); }
      if (drag.resizeHandle.includes('n')) { ny = my; nh = Math.max(2, init.y + init.h - my); }
      ...
      if (store.prefs.snapEnabled) {
        const snap = computeSnap(nx, ny, nx + nw, ny + nh, store.doc, page.elements, new Set([el.id]), store.prefs.snapTolerance / renderer.view.zoom);
        snapLines = snap.lines;
      }
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ((e.key === 'Backspace' || e.key === 'Delete') && store.selectedIds.size) {
      e.preventDefault();
  ...
  (window as unknown as { __snapLines: () => SnapLine[] }).__snapLines = () => snapLines;
}
...
function computeSnap(
  ...
  for (const px of pageX) for (const sx of sourceX) xCandidates.push({ pos: px, from: sx });
  for (const py of pageY) for (const sy of sourceY) yCandidates.push({ pos: py, from: sy });
  ...
  for (const el of elements) {
    ...
    for (const ep of ex) for (const sx of sourceX) xCandidates.push({ pos: ep, from: sx });
    for (const ep of ey) for (const sy of sourceY) yCandidates.push({ pos: ep, from: sy });
  }

  let bestDx = 0, bestX = Infinity;
  let bestDy = 0, bestY = Infinity;
  let xLine: number | null = null, yLine: number | null = null;
  for (const c of xCandidates) {
    const d = c.pos - c.from;
    if (Math.abs(d) < tol && Math.abs(d) < bestX) { bestX = Math.abs(d); bestDx = d; xLine = c.pos; }
  }
  for (const c of yCandidates) {
    const d = c.pos - c.from;
    if (Math.abs(d) < tol && Math.abs(d) < bestY) { bestY = Math.abs(d); bestDy = d; yLine = c.pos; }
  }
