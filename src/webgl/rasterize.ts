...
export function rasterizeElement(
  ...
  switch (el.type) {
    case "rect":
      drawRect(ctx, el, w, h, pxPerMm);
      break;
    case "text":
      drawText(ctx, el, w, h, pxPerMm);
      break;
    case "image":
      /* drawn via separate path with image bitmap */ break;
  }
  return canvas;
}
...
function drawText(
  ...
  switch (el.vAlign) {
    case "top":
      ...
      break;
    case "middle":
      y = (h - lines.length * lineHeight) / 2 + lineHeight * 0.8;
      break;
    case "bottom":
      y = h - (lines.length - 1) * lineHeight - lineHeight * 0.2;
      break;
  }
...
function wrapText(
  ...
  for (const para of text.split(/\n/)) {
    ...
    for (const word of words) {
      ...
      if (
        measureWithSpacing(ctx, trial, letterSpacing, sizePx) > maxWidth &&
        line.trim().length
      ) {
...
export function rasterizeImage(
  ...
  if (cellW <= 0 || cellH <= 0) return canvas;

  for (let row = 0; row < ry; row++) {
    for (let col = 0; col < rx; col++) {
      const cx = col * (cellW + gapPx);
      const cy = row * (cellH + gapPx);
      let dx = cx,
        dy = cy,
        dw = cellW,
        dh = cellH;
