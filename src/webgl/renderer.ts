...
export class Renderer {
  ...
  screenToMm(sx: number, sy: number): { x: number; y: number } {
    ...
    const cy = sy - r.top;
    return {
      x: cx / this.view.zoom + this.view.panX,
      y: cy / this.view.zoom + this.view.panY,
    };
  }
  mmToScreen(mx: number, my: number): { x: number; y: number } {
    return {
      x: (mx - this.view.panX) * this.view.zoom,
      y: (my - this.view.panY) * this.view.zoom,
    };
  }
  ...
  fitToScreen(padding = 40): void {
    const r = this.canvas.getBoundingClientRect();
    const bleedW =
      this.doc.size.width + this.doc.bleed.left + this.doc.bleed.right;
    const bleedH =
      this.doc.size.height + this.doc.bleed.top + this.doc.bleed.bottom;
    const zx = (r.width - padding * 2) / bleedW;
    const zy = (r.height - padding * 2) / bleedH;
    this.view.zoom = Math.max(0.2, Math.min(zx, zy));
    this.view.panX =
      -this.doc.bleed.left - (r.width / this.view.zoom - bleedW) / 2;
    this.view.panY =
      -this.doc.bleed.top - (r.height / this.view.zoom - bleedH) / 2;
  }
  ...
  private worldMatrix(
    ...
    return new Float32Array([m11, m21, 0, m12, m22, 0, m13, m23, 1]);
  }
  ...
  private drawElement(
    ...
    const needsRebuild =
      ...
      Math.abs(cached.pxPerMm - renderScale) / renderScale > 0.25;
    if (needsRebuild) {
  ...
  private drawSelection(
    ...
    for (const [hx, hy] of handles) {
      ...
      this.drawSolid(
        view,
        flipY,
        hx + 0.5 / this.view.zoom,
        hy + 0.5 / this.view.zoom,
        hs - 1 / this.view.zoom,
        hs - 1 / this.view.zoom,
        c,
      );
  ...
  private drawSolidMat(
    ...
    gl.uniform4f(
      gl.getUniformLocation(this.progSolid, "u_color"),
      color[0],
      color[1],
      color[2],
      color[3],
